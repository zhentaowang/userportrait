...
class JestService {
     ...
     * @return boolean
     */
    boolean update(JestClient jestClient, String indexName, String typeName, String id) throws Exception {
        JestResult jr = jestClient.execute(new Update.Builder(id).index(indexName).type(typeName).build());
        return jr.isSucceeded();
    }
